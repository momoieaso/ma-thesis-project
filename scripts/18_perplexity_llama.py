"""
Script Name: 18_perplexity_llama.py

Description:
    - This script calculates the perplexity of response texts given a set of prompts using a pretrained language model, "meta-llama/Llama-3.2-3B-Instruct".
    - It loads a model ("meta-llama/Llama-3.2-3B-Instruct") and tokenizer from Hugging Face Hub, processes pairs of prompt and response files, and computes the perplexity for each response. The results are saved in JSON format, allowing for further analysis.
...
output_dir = "../data/processed_data/perplexity/analysis_results/llama_results"
...
class PerplexityTextGenerationPipeline(TextGenerationPipeline):
    ...
    def _forward(self, model_inputs, **generate_kwargs):
        ...
        # Find the position of the last <|start_header_id|> token. 
        start_header_token_id = self.tokenizer.convert_tokens_to_ids("<|start_header_id|>")
        last_start_header_pos = (input_ids == start_header_token_id).nonzero(as_tuple=True)[1].max().item()

        # Mask labels before the last <|start_header_id|>, i.e. mask the prompt part. 
        labels[:, :last_start_header_pos + 1] = -100
        ...
        for i in range(0, total_lines, batch_size):
            ...
            for line_num, response in enumerate(batch_lines, start=i + 1):
                ...
                if response:
                    ...
                    text = f"{prompt} <|start_header_id|> {response}"
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_NAME, torch_dtype=torch.bfloat16, device_map="balanced_low_0", trust_remote_code=True
    )
